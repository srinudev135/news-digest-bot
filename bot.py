...
- Topics: GeoPolitics, AI Updates, Crypto, Trending AI on X
- Settings persisted to GitHub Gist — survive restarts
- Sends digest on startup, stays alive for interactive chat
- Telugu follow-up answers with audio playback
"""
...
DEFAULT_TOPICS = {
    ...
    # __xai__ is a special built-in key — fetched via TwitterAPI.io, not RSS/NewsAPI
    "__xai__": {
    ...
    # __xcrypto__ is a special built-in key — fetched via TwitterAPI.io
    "__xcrypto__": {
        "emoji": "₿🐦", "label": "Trending Crypto on X",
        "newsapi_q": "",
        "rss":       [],
        "builtin_x": True,
    },
...
DEFAULT_SETTINGS = {
    ...
    "active_topics":  ["geopolitics", "ai_updates", "crypto", "__xai__", "__xcrypto__"],
    "news_count":     5,
}
...
async def fetch_x_ai_tweets(count: int = 5) -> list:
    ...
    if not twitter_api_key:
        logger.warning("TWITTER_API_KEY not set — skipping X AI tweets")
        return []

    query = (
        "(ChatGPT OR OpenAI OR Gemini OR Claude OR LLM OR GPT-4 OR GPT-5 OR \"artificial intelligence\") "
        ...
        async with httpx.AsyncClient(timeout=30) as c:
            r = await c.get(
                ...
                params={
                    "query":      query,
                    "queryType":  "Top",
                    "count":      count * 3,   # fetch extra, pick best
                },
            )
        data = r.json()
        raw_tweets = data.get("tweets", [])
        logger.info(f"TwitterAPI.io returned {len(raw_tweets)} tweets")
        if raw_tweets:
            logger.info(f"Sample keys: {list(raw_tweets[0].keys())}")

    except Exception as ex:
        logger.error(f"TwitterAPI.io fetch failed: {ex}")
        return []
    ...
    for item in raw_tweets:
        ...
        ai_kw = ["ai", "chatgpt", "openai", "gemini", "llm", "gpt", "claude",
                 ...
                 "mistral", "llama", "neural", "model"]
        if not any(k in text.lower() for k in ai_kw):
            continue

        # Confirmed field names from TwitterAPI.io docs
        ...
        # Direct tweet URL from API
        tweet_url = item.get("url") or (
            f"https://x.com/{author}/status/{item.get('id','')}"
            if author else "https://x.com/search?q=AI&f=top"
        )

        # Check for article URL in tweet entities
        article_url = None
        for u in (item.get("urls") or []):
            exp = u.get("expanded_url") or u.get("url") or "" if isinstance(u, dict) else str(u)
...
async def fetch_x_crypto_tweets(count: int = 5) -> list:
    """
    Fetch top Crypto tweets/articles on X via TwitterAPI.io
    Endpoint: GET https://api.twitterapi.io/twitter/tweet/advanced_search
    """
    twitter_api_key = os.environ.get("TWITTER_API_KEY", "")
    if not twitter_api_key:
    ...
    try:
        ...
        data       = r.json()
        raw_tweets = data.get("tweets", [])
        logger.info(f"TwitterAPI.io Crypto returned {len(raw_tweets)} tweets")
    except Exception as ex:
        logger.error(f"TwitterAPI.io Crypto fetch failed: {ex}")
        return []
    ...
    for item in raw_tweets:
        ...
        tweet_url = item.get("url") or (
            f"https://x.com/{author}/status/{item.get('id','')}"
            if author else "https://x.com/search?q=crypto&f=top"
        )
    ...
    logger.info(f"Returning {len(results)} Crypto tweets")
    return results


async def build_x_crypto_section(tweets: list) -> tuple:
    """Build the Trending Crypto on X digest message + keyboard."""
    divider  = "―" * 22
    lines    = "\n".join(f"{i}. {h(t['title'])}" for i, t in enumerate(tweets, 1))
    text     = f"{divider}\n₿🐦 <b>TRENDING CRYPTO ON X</b>\n<i>📰 = article link  🐦 = tweet link</i>\n{divider}\n\n{lines}"
    ask_row  = [InlineKeyboardButton(f"💬 {i}", callback_data=f"ask|__xcrypto__|{i-1}") for i in range(1, len(tweets)+1)]
    link_row = [InlineKeyboardButton(f"🔗 {i}", url=tweets[i-1]["link"])                for i in range(1, len(tweets)+1)]
    return text, InlineKeyboardMarkup([ask_row, link_row])
...
async def send_digest(app: Application, chat_id: int):
    ...
    for key in settings["active_topics"]:
        ...
        # Special: Trending AI on X — fetched via TwitterAPI.io
        if key == "__xai__":
        ...
        # Special: Trending Crypto on X — fetched via TwitterAPI.io
        if key == "__xcrypto__":
            if not os.environ.get("TWITTER_API_KEY", ""):
                continue
            crypto_tweets = await fetch_x_crypto_tweets(count=settings["news_count"])
            if not crypto_tweets:
                await app.bot.send_message(chat_id=chat_id, parse_mode="HTML",
                    text=f"₿🐦 <b>TRENDING CRYPTO ON X</b>\n\n<i>నేడు వార్తలు అందుబాటులో లేవు.</i>")
                continue
            todays_digest["__xcrypto__"] = crypto_tweets
            text, kb = await build_x_crypto_section(crypto_tweets)
            await app.bot.send_message(chat_id=chat_id, text=text, parse_mode="HTML",
                                       reply_markup=kb, disable_web_page_preview=True)
            continue
...
async def handle_story_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if section_key == "__xai__":
        cfg = {"emoji": "🤖🐦", "label": "Trending AI on X"}
    elif section_key == "__xcrypto__":
        cfg = {"emoji": "₿🐦", "label": "Trending Crypto on X"}
    else:
        cfg = topics.get(section_key, {})
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_story_callback,    pattern=r"^ask\|"))
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern="^set_"))
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern="^topic_"))
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern="^edit_time"))
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern="^add_time$"))
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern="^remove_time$"))
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern="^set_count"))
