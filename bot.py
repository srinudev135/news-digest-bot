...
async def fetch_x_ai_tweets(count: int = 5) -> list:
    ...
    try:
        ...
        for item in items:
            ...
            # ── Text: try every known field name ──
            text = (
                item.get("text") or
            ...
            author = (
                (author_obj.get("userName") or author_obj.get("screen_name") or
                 author_obj.get("username") or author_obj.get("handle"))
                if isinstance(author_obj, dict) else ""
            ) or item.get("username") or item.get("screen_name") or item.get("authorName") or ""
            ...
            tweet_id = (
                ...
                item.get("id_str") or
                item.get("statusId") or ""
            )

            tweet_url = (
                f"https://x.com/{author}/status/{tweet_id}"
                if (author and tweet_id)
                else item.get("url") or item.get("tweetUrl") or item.get("tweet_url") or "https://x.com/search?q=AI&f=top"
            )

            # ── Article URLs embedded in tweet ──
            article_url = None
            # Try multiple possible URL field locations
            url_candidates = (
                item.get("urls") or
                item.get("extendedEntities", {}).get("urls") or
                item.get("entities", {}).get("urls") or
                []
            )
            if isinstance(url_candidates, list):
                for u in url_candidates:
                    if isinstance(u, dict):
                        expanded = u.get("expanded_url") or u.get("expandedUrl") or u.get("url") or ""
                    else:
                        expanded = str(u)
